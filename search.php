...
$total_record_sql = "SELECT * FROM details d  JOIN product p ON d.SKU = p.Product_SKU   JOIN image i ON d.SKU = i.SKU_code
WHERE d.title LIKE '%{$search}%' OR  p.Type LIKE '%{$search}%' OR d.title LIKE '%{$search}%' OR d.SKU LIKE '%{$search}%' ";
...
<script type="text/Javascript"></script>
    ...
    <title>MN/<?php  echo 'SEARCH/'.strtoupper($search);?>
    </title>
...
if(isset($_POST['l_submit'])){
  ...
  // header('Location:'.$hostname.'search.php?search='.$search ) ;
      //Method 2 of re-directing
  $url = $hostname.'search.php?search='.$search ;
...
//get all the itmes with the search term


    $sql = "SELECT * FROM details d  JOIN product p ON d.SKU = p.Product_SKU   JOIN image i ON d.SKU = i.SKU_code
            WHERE d.title LIKE '%{$search}%' OR  p.Type LIKE '%{$search}%' OR d.SKU LIKE '%{$search}%'
            LIMIT {$offset} , {$limit} ";
    ...
    if(mysqli_num_rows($result) >0){
        ...
        $type = $row['Type'] ; 
        $cat_sql = "SELECT * FROM category_table WHERE `Cat-Code` = '{$type}' " ;
    ...
    ?>
             ...
             <div class="items">
               <a href="product_page.php?category=<?php echo  $category_name ;?>&CATEGORY=<?php echo $type ;?>&SKU=<?php echo $row['SKU_code'];?>">
               <img src="Database/images/<?php echo $category_name.'/'.$row['filename'].'/'.'pic.jpg'; ?>"></a>
               <div class="buy">
               <a class="buybtn" href="buynow.php?category=<?php echo $category_name ;?>&CATEGORY=<?php echo $row['Type'] ;?>&SKU=<?php echo $row['SKU_code'];?>" ><h2>BUY NOW</h2></a>
   ...
   <?php     
          ...
          if($NoOfPages > 0){
              ...
              if($pageNO>1){
                  $prev = $pageNO-1 ;
                echo '<span><a href="search.php?search='.$search.'&page='.$prev.'"> << </a></span>' ;
                
              }
              for( $i=$pageNO ; $i < $pageNO+4 && $i<= $NoOfPages; $i++){
                  ?>

              <span><a href="search.php?search=<?php echo $search ; ?>&page=<?php echo $i; ?>"><?php echo $i; ?>
                  </a></span>
             
    <?php 
        } 

        if($pageNO < $NoOfPages-4){
            $next = $pageNO+1 ;
          echo '<span><a href="search.php?search='.$search.'&page='.$next.'"> >> </a></span>' ;
          
        }
    }else{echo '<h2 class="norecords"> Nothing related to '.$search.' is found.</h2>';} 
     ?> <!--End of 1st if and while-->
